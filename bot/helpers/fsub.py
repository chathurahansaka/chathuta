...
async def ForceSub(bot: Client, event: Message):
    try:
        invite_link = await bot.create_chat_invite_link(chat_id=(int(Config.FSUB_CHANNEL) if Config.FSUB_CHANNEL.startswith("-100") else Config.FSUB_CHANNEL))
    except FloodWait as e:
        await asyncio.sleep(e.x)
        fix_ = await ForceSub(bot, event)
        return fix_
    except Exception as err:
        print(f"Unable to do Force Subscribe to {Config.FSUB_CHANNEL}\n\nError: {err}\n\nContact Support Group: https://t.me/slbotzone")
        return 200
    try:
        user = await bot.get_chat_member(chat_id=(int(Config.FSUB_CHANNEL) if Config.FSUB_CHANNEL.startswith("-100") else Config.FSUB_CHANNEL), user_id=event.from_user.id)
